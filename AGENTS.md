...
5. Fetch tags from `origin`, then identify the highest valid SemVer tag reachable from `HEAD`. Create the next minor version by incrementing the minor component and resetting the patch component to zero (for example, `v1.4.7` becomes `v1.5.0`, while `1.4.7` becomes `1.5.0`). Preserve whether the existing tags use a `v` prefix. If there is no valid SemVer tag, or the tag format is ambiguous, stop and ask the user for the initial/desired version.
